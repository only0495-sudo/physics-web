(function () {
  ...
  function getRole() {
    ...
    return query === "teacher" || query === "student" ? query : storage.get("physics-role", "teacher");
  }
  ...
  function installCanvasClarity() {
    const proto = window.CanvasRenderingContext2D?.prototype;
    if (!proto || proto.__pcClarityPatched) return;
    ...
    const outlineFor = (fillStyle) => {
      const rgb = parseColor(fillStyle);
      if (!rgb) return "rgba(255,255,255,.9)";
      const luminance = (rgb[0] * .2126 + rgb[1] * .7152 + rgb[2] * .0722) / 255;
      return luminance > .58 ? "rgba(7,18,38,.88)" : "rgba(255,255,255,.92)";
    };
    proto.fillText = function (...args) {
      if (!document.documentElement.classList.contains("pc-clarity-mode")) return originalFillText.apply(this, args);
      this.save();
      const size = Number.parseFloat(String(this.font).match(/([\d.]+)px/)?.[1] || "0");
      if (size && size < 14) this.font = String(this.font).replace(/([\d.]+)px/, "14px");
      this.lineJoin = "round";
      this.miterLimit = 2;
      this.lineWidth = Math.max(2.5, Math.min(4, (size || 14) * .2));
      this.strokeStyle = outlineFor(this.fillStyle);
      originalStrokeText.apply(this, args);
      originalFillText.apply(this, args);
      this.restore();
    };
    Object.defineProperty(proto, "__pcClarityPatched", { value: true });
  }

  function buildShell() {
    installCanvasClarity();
    const type = detectDevice();
    const clarityOn = storage.get("physics-clarity", "on") !== "off";
    document.documentElement.classList.toggle("pc-clarity-mode", clarityOn);
    if (embedded) {
    ...
    shell.innerHTML = `
      <nav class="pc-dock" aria-label="物理教學工具列">
        <div class="pc-brand" aria-label="裝置已辨識為${device.label}">
          ...
          <span class="pc-brand-copy"><strong>${role === "teacher" ? "課堂演示" : "物理探究"}</strong><span class="pc-device-label">${device.label}</span></span>
        </div>
        ...
        <button class="pc-dock-action pc-clarity-button" type="button" aria-pressed="${clarityOn}" title="加強投影可讀性（Alt+C）"><span class="pc-action-symbol" aria-hidden="true">Aa</span><span class="pc-dock-label">清晰</span></button>
        <button class="pc-dock-action pc-focus-button" type="button" aria-expanded="false" title="顯示本頁觀察重點（Alt+K）"><span class="pc-action-symbol" aria-hidden="true">◎</span><span class="pc-dock-label">重點</span></button>
      ...
      <section class="pc-focus-card" aria-hidden="true" aria-labelledby="pc-focus-title">
        <div class="pc-focus-copy">
          <span>${escapeHtml(activity.category)} · 現在要看什麼</span>
          <h2 id="pc-focus-title">${escapeHtml(activity.summary)}</h2>
          <p><strong>請全班注意：</strong>${escapeHtml(activity.question)}</p>
        </div>
        <button class="pc-focus-close" type="button" aria-label="關閉觀察重點">×</button>
      </section>
  ...
  function bindShell(shell) {
    ...
    const clarityButton = shell.querySelector(".pc-clarity-button");
    const focusButton = shell.querySelector(".pc-focus-button");
    const focusCard = shell.querySelector(".pc-focus-card");
    ...
    function toggleClarity() {
      const active = !document.documentElement.classList.contains("pc-clarity-mode");
      document.documentElement.classList.toggle("pc-clarity-mode", active);
      clarityButton.setAttribute("aria-pressed", String(active));
      storage.set("physics-clarity", active ? "on" : "off");
      window.dispatchEvent(new Event("resize"));
      showToast(active ? "已加強文字、控制項與向量對比" : "已恢復原始顯示密度");
    }

    function setFocusCard(open) {
      focusCard.classList.toggle("pc-open", open);
      focusCard.setAttribute("aria-hidden", String(!open));
      focusButton.setAttribute("aria-expanded", String(open));
      if (open) focusCard.querySelector(".pc-focus-close")?.focus();
    }
    ...
    clarityButton.addEventListener("click", toggleClarity);
    focusButton.addEventListener("click", () => setFocusCard(!focusCard.classList.contains("pc-open")));
    shell.querySelector(".pc-focus-close").addEventListener("click", () => setFocusCard(false));
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (event.key === "Escape" && focusCard.classList.contains("pc-open")) setFocusCard(false);
      ...
      if (key === "c") { event.preventDefault(); toggleClarity(); }
      if (key === "k") { event.preventDefault(); setFocusCard(!focusCard.classList.contains("pc-open")); }
